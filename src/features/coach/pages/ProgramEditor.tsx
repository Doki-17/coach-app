...
export default function ProgramEditor({ mode = 'edit' }: { mode?: ProgramEditorMode }) {
  ...
  // Removing a week also drops that week's progression entry from every
  // exercise that tracks one, so exercise rows stay aligned with the weeks
  // that remain. A program always keeps at least one week, so the last one
  // can't be deleted from here.
  const deleteWeek = (weekIndex: number) => {
    if (weeks.length <= 1) return;
    setWeeks(weeks.filter((_, i) => i !== weekIndex).map((week, i) => ({ ...week, id: i + 1 })));
    setCategories(categories.map((cat) => {
      const type = getCategoryType(cat.categoryType);
      if (cat.categoryType === LEGACY_CATEGORY_TYPE_ID || type.progressionColumns.length === 0) return cat;
      return {
        ...
        exercises: cat.exercises.map((ex) => ({
          ...ex,
          progression: ex.progression.filter((_, i) => i !== weekIndex),
        })),
      };
    }));
  };

  // Inserts a new blank week directly below this one, rather than always
  // at the very end like the "Add Week" button below the table - handy for
  // slotting a week into the middle of a program. Mirrors addWeek's
  // per-category progression sync, just spliced in at a specific spot.
  const insertWeekAfter = (weekIndex: number) => {
    const newWeeks = [...weeks];
    newWeeks.splice(weekIndex + 1, 0, { id: 0, am: Array(7).fill(''), pm: Array(7).fill(''), showPm: false });
    setWeeks(newWeeks.map((week, i) => ({ ...week, id: i + 1 })));
    setCategories(categories.map((cat) => {
      const type = getCategoryType(cat.categoryType);
      if (cat.categoryType === LEGACY_CATEGORY_TYPE_ID || type.progressionColumns.length === 0) return cat;
      const blankWeek = Object.fromEntries(type.progressionColumns.map((c) => [c.key, '']));
      return {
        ...
        exercises: cat.exercises.map((ex) => {
          ...
          progression.splice(weekIndex + 1, 0, { ...blankWeek });
  ...
  // Inserts a copy of a week (its AM/PM text included) directly below the
  // original, and a matching duplicate progression entry in every category
  // that tracks per-week progression, so exercise rows stay aligned with
  // the weeks. Weeks are renumbered afterward so `id` always matches
  // position, the same as addWeek/deleteWeek.
  const duplicateWeek = (weekIndex: number) => {
    const original = weeks[weekIndex];
    const duplicate = { ...original, am: [...original.am], pm: [...original.pm] };
    const newWeeks = [...weeks];
    newWeeks.splice(weekIndex + 1, 0, duplicate);
    setWeeks(newWeeks.map((week, i) => ({ ...week, id: i + 1 })));
    setCategories(categories.map((cat) => {
      const type = getCategoryType(cat.categoryType);
      if (cat.categoryType === LEGACY_CATEGORY_TYPE_ID || type.progressionColumns.length === 0) return cat;
      return {
        ...
        exercises: cat.exercises.map((ex) => {
          const progression = [...ex.progression];
          const copiedWeek = { ...progression[weekIndex] };
          progression.splice(weekIndex + 1, 0, copiedWeek);
          return { ...ex, progression };
        }),
      };
    }));
  ...
  // Inserts a new blank exercise row directly below this one, rather than
  // always at the end like the "Add Exercise Row" button below the table.
  const insertExerciseAfter = (catIndex: number, exIndex: number) => {
    const newCategories = [...categories];
    const category = newCategories[catIndex];
    const type = getCategoryType(category.categoryType);
    const weekCount = progressionWeekCount(type, weeks.length);
    ...
    exercises.splice(exIndex + 1, 0, {
      id: crypto.randomUUID(),
      name: '',
      fixed: {},
      progression: Array.from({ length: weekCount }, () => ({ ...blankWeek })),
    });
    newCategories[catIndex] = { ...category, exercises };
    setCategories(newCategories);
  };
          ...
          <table className="w-full text-sm text-center border-collapse table-fixed">
            <thead className="bg-blue-50">
              <tr>
                ...
                <th className="border p-3 w-20 exclude-from-png"></th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <React.Fragment key={week.id}>
                    <tr>
                      ...
                      <td className="border p-0 text-center exclude-from-png" rowSpan={week.showPm ? 2 : 1}>
                        <div className="flex items-center justify-center gap-2">
                          <button
                            type="button"
                            onClick={() => insertWeekAfter(wIndex)}
                            title="Add week below"
              ...
              <div key={category.id}>
                <div className={`flex items-center gap-3 mb-4 p-3 rounded-lg border-l-4 ${visual.headerBg} ${visual.headerBorder}`}>
                  ...
                  <button
                    ...
                    className="exclude-from-png shrink-0 text-gray-400 hover:text-red-500"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                    ...
                    {multiField ? (
                      <>
                        <tr>
                          ...
                          <th className="border p-3 w-20 exclude-from-png" rowSpan={2}></th>
                        </tr>
                        <tr>
                          {Array.from({ length: weekCount }, (_, i) => (
                    ...
                    {category.exercises.map((ex, exIndex) => (
                      <tr key={ex.id} className="group hover:bg-gray-50">
                        ...
                        <td className="border p-0 text-center exclude-from-png">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              onClick={() => insertExerciseAfter(catIndex, exIndex)}
                              title="Add row below"
                              className="text-gray-400 hover:text-blue-600"
                            >
                              <Plus className="w-4 h-4" />
