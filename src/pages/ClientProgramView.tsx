...
import { ArrowLeft, Pencil, Plus, History as HistoryIcon, MapPin, X } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="flex justify-between items-center mb-2">
        ...
        {hasSavedProgram ? (
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={() => setShowHistory(true)} className="bg-white border border-gray-300 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-50">
            ...
            <button onClick={() => navigate(`/client/${clientId}/new`)} className="bg-white border border-gray-300 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-50">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex-1 flex flex-wrap justify-center gap-x-8 gap-y-2 min-w-[180px]">
            {client?.weightKg != null && (
              <div className="text-sm text-center">
                ...
                <p className="font-medium text-gray-800">{client.weightKg} kg</p>
              </div>
            )}
            {client?.heightCm != null && (
              <div className="text-sm text-center">
                <span className="text-gray-400">Height</span>
                <p className="font-medium text-gray-800">{client.heightCm} cm</p>
              </div>
            )}
            {bmi != null && (
              <div className="text-sm text-center">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="text-xs text-gray-500 border border-gray-300 rounded-lg px-3 py-1 flex items-center gap-1 hover:bg-gray-50"
            >
        ...
        {client?.remarks && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">Notes / Remarks</span>
            <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap">{client.remarks}</p>
          </div>
        )}
      </div>

      <h2 className="text-lg font-semibold text-gray-700 mb-1">Current Program</h2>
                ...
                {history.map((v) => (
                  <li key={v.id} className="border border-gray-200 rounded-lg p-3">
                    ...
                    <div className="flex gap-4">
                      ...
                      <button onClick={() => handleRestore(v.id)} className="text-xs font-medium text-gray-600 hover:underline">
                        Restore
                      </button>
                    </div>
                  </li>
      ...
      {viewingVersion && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-6" onClick={() => setViewingVersion(null)}>
          <div className="bg-white rounded-xl max-w-5xl w-full max-h-[85vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
